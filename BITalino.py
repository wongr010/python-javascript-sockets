...
class BITalino:
    ...
    def read(self, nSamples=100):
        ...
        while len(Data) < dataSize:
            ...
            dataAcquired = self.decode(Data, nSamples)
            return dataAcquired

    def decode(self, data, nSamples):
        ...
        Kwargs:
            ...
            nSamples (int): number of samples contained in data

        Output:
            res(array): data unpacked
        """

        res = numpy.zeros(((len(self.analogChannels) + 5), nSamples))
        ...
        while 1:
            ...
            try:
                ...
                # Digital 0
                Digital0 = int(data[j + self.number_bytes - 2].encode("hex"), 16) >> 7 & encode01
                res[line, col] = Digital0
                line += 1

                # Digital 1
                Digital1 = int(data[j + self.number_bytes - 2].encode("hex"), 16) >> 6 & encode01
                res[line, col] = Digital1
                line += 1

                # Digital 2
                Digital2 = int(data[j + self.number_bytes - 2].encode("hex"), 16) >> 5 & encode01
                res[line, col] = Digital2
                line += 1

                # Digital 3
                Digital3 = int(data[j + self.number_bytes - 2].encode("hex"), 16) >> 4 & encode01
                res[line, col] = Digital3
                line += 1

                if self.number_bytes >= 3:
                    # Analog 0
                    Analog0 = (int(data[j + self.number_bytes - 2].encode("hex"), 16) & encode0F) << 6 | (
                                int(data[j + self.number_bytes - 3].encode("hex"), 16) & encodeFC) >> 2
                    res[line, col] = Analog0
                    line += 1

                if self.number_bytes >= 4:
                    # Analog 1
                    Analog1 = (int(data[j + self.number_bytes - 3].encode("hex"), 16) & encode03) << 8 | (
                                int(data[j + self.number_bytes - 4].encode("hex"), 16) & encodeFF)
                    res[line, col] = Analog1
                    line += 1

                if self.number_bytes >= 6:
                    # Analog 2
                    Analog2 = (int(data[j + self.number_bytes - 5].encode("hex"), 16) & encodeFF) << 2 | (
                                int(data[j + self.number_bytes - 6].encode("hex"), 16) & encodeC0) >> 6
                    res[line, col] = Analog2
                    line += 1

                if self.number_bytes >= 7:
                    # Analog 3
                    Analog3 = (int(data[j + self.number_bytes - 6].encode("hex"), 16) & encode3F) << 4 | (
                                int(data[j + self.number_bytes - 7].encode("hex"), 16) & encodeF0) >> 4
                ...
                if self.number_bytes >= 8:
                    ...
                    # Analog 4
                    Analog4 = (int(data[j + self.number_bytes - 7].encode("hex"), 16) & encode0F) << 2 | (
                                int(data[j + self.number_bytes - 8].encode("hex"), 16) & encodeC0) >> 6
                    res[line, col] = Analog4 * (1023 / 63)
                    line += 1

                    # Analog 5
                    if numpy.shape(res)[0] == 12:
                        Analog5 = int(data[j + self.number_bytes - 8].encode("hex"), 16) & encode3F
